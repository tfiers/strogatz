...
\captionn{Phase space diagrams for different predator decay rates $\bm{c}$}{Fixed points are indicated with colored circles. Their manifolds of attraction are given the same color. (At $c = 0$, the entire $y$-axis consists of fixed points). The limit cycle for $c_h = 0.446... < c < 0.55$, its basin of attraction, and its period $T$, are indicated in purple.}
